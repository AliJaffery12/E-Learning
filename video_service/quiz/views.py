...
def create_quiz(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_quiz = Quiz(quiz_title=quiz_name, num_questions=num_questions,subject=subject)
            new_quiz.save()
...
#@login_required
# def index(request):
 #   #ss=request.GET.get('name')
  #  #all_quiz_list = Quiz.objects.all()
  #  all_quiz_list=Quiz.objects.filter(subject__isnull=False).distinct()
   # context = {
...
def CourseListView(request):
    ...
    #coursees=Course.objects.filter(request.session.get('cart'))
    context={
        'course':myCourses
    }
    #template_name='quiz/coursesList.html'
    return render(request,'quiz/coursesList.html',context)
...
# view that receives info from u


# def single_slug(request):
    
#     subjects=[c.name for c in Subject.objects.all()]
#     if quiz_slug in subjects:
#         matching_subjects= Quiz.objects.filter(subject__name=quiz_slug)
#         series_url={}

#         for m in matching_subjects.all():
#             part_one=Quiz.objects.filter(subject__name= m.quiz_title)
#             series_url[m]=part_one.quiz_title
#         return render(request=request,template_name='courses/selected_quiz.html',context={"subjects_match":matching_subjects,"quiz_part":series_url})
...
def results(request, quiz_id):
    ...
    if accuracy >= .75 and total_questions>5:
        accuracy_over_75 = True
       
        #return render(request,'quiz/Mycertificate.html')
        return CourseListView(request)
        #return student_course_pdf(request)
...
def student_course_pdf(request,*args,**kwargs):
    pk=kwargs.get('pk')
    course=get_object_or_404(Course,pk=pk)
    #user=get_object_or_404(User,pk=pk)
    #current_user=request.user
    #user=current_user.id
    template_path = 'quiz/Mycertificate.html'
    context = {'course': course,'user':request.user.username}
...
def index(request):
    ...
    if subjectID:
        ...
        all_quiz_list=Quiz.get_all_Quiz_by_subjectid(subjectID)
    else:
         all_quiz_list=Quiz.objects.all()

    data={}
    data['all_quiz_list']=all_quiz_list
